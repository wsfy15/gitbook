...
对于经常访问的表或者表数据不多，可以直接在被驱动表上建索引，这时就可以直接转成BKA算法了。但是有些情况直接键索引就比较浪费，例如`t2`有100万行数据，假如下面的语句是个低频语句，为表`t2`建立字段`b`上的索引就不合适了。
...
对于Q1，驱动表是表a，被驱动表是表b，由于表b的`f1`和`f2`字段上没有索引，所以使用的是Block Nexted Loop Join（简称BNL） 算法。执行流程如下：
